...
#spectra = np.stack((spec1,spec2,spec3), axis=-1)
print('\n' + sys.argv[1])

resp, model, r_tmp = fitting_scripts.flux_calib(spectra, sys.argv[1])
...
inp = sys.argv[1]
n = inp.split('/')[-1]
StN = np.sum( spectra[:,1][(spectra[:,0] >= 4500.0) & (spectra[:,0] <= 4750.0)] / spectra[:,2][(spectra[:,0] >= 4500.0) & (spectra[:,0] <= 4750.0)] ) / spectra[:,1][(spectra[:,0] >= 4500.0) & (spectra[:,0] <= 4750.0)].size
axes2.text(  4500, 0.4, 'StN = %.1f'%(StN), bbox={'edgecolor':'white', 'facecolor':'white', 'alpha':1, 'pad':2}, fontsize = 8)
#######################################
s2_c, s3_c = spec2*resp, spec3*resp
axes1.plot(spec1, spec2, color = 'red', lw = 1)
#axes2.plot(spec1, resp/max(resp))
#axes2.plot(spec1, tmp/max(tmp))
#axes2.plot(spec1, r_tmp/max(resp), color = 'black', alpha = 0.5, zorder = 0, lw = 0.5)
